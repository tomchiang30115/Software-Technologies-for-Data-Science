...
def handle_delete_session(iuser, imagic):
    ...
    session_end = access_database_with_result(
        dbfile,
        f"""SELECT u.userid, s.magic FROM users u
        INNER JOIN session s ON u.userid = s.userid
        WHERE u.username = '{iuser}' AND s.magic = '{imagic}' AND s.end = 0""",
    )
    userid = session_end[0][0]
    magic = session_end[0][1]
    access_database(
        dbfile, f"""UPDATE session SET end = {end_delete}
        WHERE userid = {userid} AND magic = '{magic}' AND end = 0"""
    )
    return
...
def handle_login_request(iuser, imagic, parameters):
    ...
    if username_input != " ".join(re.findall(r"[0-9a-z ]+", username_input)):
        response.append(
        ...
        return [user, magic, response]
    elif password_input != " ".join(re.findall(r"[0-9a-z ]+", password_input)):
        response.append(
            build_response_refill(
...
def handle_add_request(iuser, imagic, parameters):
    ...
    if location_input != " ".join(re.findall(r"[0-9a-z ]+", location_input)):
        response.append(
            build_response_refill(
        ...
        return [user, magic, response]
    elif occupancy_input != " ".join(re.findall(r"[0-9a-z ]+", occupancy_input)):
        response.append(
            build_response_refill(
        ...
        return [user, magic, response]
    elif type_input != " ".join(re.findall(r"[0-9a-z ]+", type_input)):
        response.append(
            build_response_refill(
...
def handle_logout_request(iuser, imagic, parameters):
    ...
    session_end_query = access_database_with_result(
        dbfile,
        f"""SELECT u.userid, s.magic FROM users u
    INNER JOIN session s ON u.userid = s.userid
    ...
    access_database(
        dbfile,
        f"""UPDATE session SET end = {end_logout}
        WHERE magic = '{imagic}'
        AND userid = {userid}""",
    )
